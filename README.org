...
#+BEGIN_SRC shell
  export PATH="/cross-tools/bin:/tools/bin:$PATH"
#+END_SRC
*** CPU configuration
**** x86_64
...
#+BEGIN_SRC shell
  ...
  mkdir -p "/etc/acpid"
  install -Dm755 /files/mdev.run /etc/sv/mdev/run
  install -Dm644 /files/mdev.conf /etc/mdev.conf
#+END_SRC
*** Check
...
#+END_SRC
Create files for poweroff and reboot:
#+BEGIN_SRC shell
  cp /files/shutdown.sh /sbin/shutdown
  cp /files/reboot.sh   /sbin/reboot
#+END_SRC
